...
class FRCNN(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"    : 'frcnn_weights/best_epoch_weights.pth',
        "classes_path"  : 'frcnn_data/MOCS_classes.txt',
        #---------------------------------------------------------------------#
...
################################### json ################################
    ...
    def cal_json_str(self, image):
        ...
        with torch.no_grad():
            ...
            top_label   = np.array(results[0][:, 5], dtype = 'int32')
            top_label = list(top_label)
            
            json_str = np.zeros(len(self.class_names))
            for idx in range(len(json_str)):
                json_str[idx] = top_label.count(idx)
